...
/*
在序列中间频繁添加删除元素，最好用list，两端的话用deque
*/
int main()
{
/**************************************************初始化**********************************************************/
/*常见操作：size(),empty(),push_back,pop_back(),insert(),erase(),clear(),swap()*/
	...
	vector<string> temp5={"i love you","魏泷芩"};                 //泛型，初始化，有两个元素的string数组，相当于二维的char
...
/***********************empty()和pop_back()和push_back()和earse()和clear()***************************************/
	...
	*/
	...
	insert()版本：
		...
		（3）insert(p, il)    //将列表中的元素插入，返回第一个插入元素的迭代器（不懂）
		                      //关于迭代器确定范围都是左闭右开！！！！
	*/
	...
	temp1.insert(temp1.begin()+2,6);//（1）
...
/********************************************************************algoriothm****************************************************************/
	...
	//查找
	auto iter=find(temp1.begin(),temp1.end()-5,8);              //从begin(),到end()（不包括它）位置找数字8，找到返回这个位置的指针
	cout<<*iter;
}
