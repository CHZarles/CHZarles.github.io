/*C++中string是标准库中一种容器,相当于保存元素类型为char的vector容器(自己理解)
这个类提供了相当丰富的函数来完成对字符串操作,以及与C风格字符串之间转换下面是对
string一些总结<引用>*/
//string和vector是一样的，支持快速随机访问，在尾部之外的其他的位置插入都很慢
...
int main()
{
	...
	cout << "the length of str1：" << str1.size() << endl;
	...
	auto ret1=str1.insert(6,"#");							//参数为下标的(插入数组第六个位置)，插入的是字符串，返回的是插入之后str1的引用,记住字符串下标是从0开始的
	cout << "after inserting, str1 is : " << str1 << endl;
	cout << "插入返回的值为：" << ret1 << endl;
	
	auto  ret2=str1.erase(str1.begin()+5);					//接受迭代器版本的，返回删除元素之后的迭代器,删除第五个字符
	cout << "after earsing , str1 is: " << str1 << endl;
	cout << "返回的迭代器指向元素为：" << *ret2 << endl;
	
	auto ret3=str1.erase(5,1); 								//接受下标参数的，,接受删除长度，返回删除元素之后的str1的引用
	cout << "after earsing , str1 is: " << str1 << endl;
	cout << "返回的迭代器指向元素为：" << ret3 << endl;
	...
	//举例几种常见的：
	str1.replace(5,1,"###");									////(pos, len, str)形式,中间参数为1才不会把其他字符占掉 
	...
	*/
	auto pos = str1.find("#");
	cout << "str2 is at the pos : " << pos << " of str1" << std::endl;
	string numbers("01234556789");
	string name("ry2d2");
    auto pos1 = name.find_first_of(numbers);
	cout << "number first at the pos : " << pos1 << " of name" << std::endl;
