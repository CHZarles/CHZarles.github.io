---
title: python的垃圾回收机制/传值/赋值原理[转]
comments: true
copyright: false
date: 2020-07-24 11:14:59
tags: python
categories: python基本语法
photo:
top:
cover: http://p2.ifengimg.com/a/2019_28/37b93132096ddfa_size334_w1920_h1280.jpg
toc: true
---

## [一 引入](https://zhuanlan.zhihu.com/p/108683483)

 解释器在执行到定义变量的语法时，会申请内存空间来存放变量的值，而内存的容量是有限的，这就涉及到变量值所占用内存空间的回收问题，当一个变量值没有用了（简称垃圾）就应该将其占用的内存给回收掉，那什么样的变量值是没有用的呢？

 单从逻辑层面分析，我们定义变量将变量值存起来的目的是为了以后取出来使用，而取得变量值需要通过其绑定的直接引用（如x=10，10被x直接引用）或间接引用（如l=[x,]，x=10，10被x直接引用，而被容器类型l间接引用），所以当一个**变量值不再绑定任何引用**时，**我们就无法再访问到该变量值了，该变量值自然就是没有用的，就应该被当成一个垃圾回收**。

Cpython解释器提供了自动的垃圾回收机制来帮我们解决了这件事。

![img](https://pic3.zhimg.com/v2-fdd6ee1176778e295514ae8f3d0d6053_r.jpg)
...
![img](https://pic1.zhimg.com/80/v2-1dade7ff0e1a23520534e517b2cde6b1_1440w.jpg)

## 四、理解GC原理需要储备的知识

## 4.1、堆区与栈区

 在定义变量时，**变量名**与**变量值**都是需要存储的，分别对应内存中的两块区域：**堆区与栈区**。

```python
# 1、变量名与值内存地址的关联关系存放于栈区 注意 是 关联关系
...
![img](https://pic4.zhimg.com/v2-73d5845ef7dc5a403333d2099d456fc0_r.jpg)

当我们执行x=y时，内存中的栈区与堆区变化如下

![img](https://picb.zhimg.com/v2-9ec5b20fd2c2d2e9c90194d57bcf7778_r.jpg)
...
![img](https://pic2.zhimg.com/v2-4240913539ce0352a89c4044c2a418e9_r.jpg)

## 五、垃圾回收机制原理分析

 Python的GC模块主要运用了**“引用计数”**（reference counting）来跟踪和回收垃圾。在引用计数的基础上，还可以通过“标记-清除”（mark and sweep）解决容器对象可能产生的循环引用的问题，并且通过“分代回收”（generation collection）以空间换取时间的方式来进一步提高垃圾回收的效率。

![img](https://pic2.zhimg.com/80/v2-d0747b871d71091eff8ab5b0ebb6b3b0_1440w.jpg)
...
![img](https://pic2.zhimg.com/v2-89915f2dc64db1fe79ce739a233bb09a_r.jpg)
...
![img](https://pic4.zhimg.com/v2-8a46e8993c193017bbbbca370ce6970b_r.jpg)
...
![img](https://picb.zhimg.com/v2-6d0ed3fa462194215235ae22888b373a_r.jpg)

值18的引用计数一旦变为0，其占用的内存地址就应该被解释器的垃圾回收机制回收

## 5.2、引用计数的问题与解决方案

![img](https://pic3.zhimg.com/80/v2-2876d1bad8466226f58b1edcb84714a0_1440w.jpg)
...
![img](https://pic1.zhimg.com/v2-ba7f7e5f5f42b44a24123a48996ac21a_r.jpg)
...
![img](https://pic1.zhimg.com/v2-6a71b134e25254477bef148daf75c328_r.jpg)
...
![img](https://pic4.zhimg.com/80/v2-3c62f9500a8809200caa264257b93b14_1440w.jpg)
...
![img](https://picb.zhimg.com/v2-869f9debccd400e67a57552f72211921_r.jpg)
...
![img](https://pic3.zhimg.com/v2-2458685b385b338192d6b90628ed92a8_r.jpg)
...
辅助阅读：[python变量讲解](https://zhuanlan.zhihu.com/p/108679428)
